...
export default function ProjectDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PortalShell><section className="portal-content">
    ...
    {!project ? <p className="portal-loading">Loading project…</p> : <>
      ...
      <div className="portal-grid"><section className="portal-section"><h2>Files</h2>{files.length ? files.map((file) => <div className="file-row" key={file.id}><span><strong>{file.display_name || file.filename}</strong><small>{file.category.replaceAll("_", " ")}</small>{(file.category === "preview" || file.category === "approval") && !file.preview_supported && <small>This file cannot be previewed in the browser. Download it instead.</small>}</span><div><button onClick={() => void downloadProtectedFile(file.download_url, file.filename)}>Download</button>{file.preview_supported && file.preview_url && <button onClick={() => void previewProtectedFile(file.preview_url!)}>Preview</button>}{file.category === "approval" && file.pending_approval && <><button onClick={() => void respondToApproval(file.id, "approved")}>Approve</button><button onClick={() => void respondToApproval(file.id, "changes_requested")}>Request changes</button></>}</div></div>) : <p className="portal-muted">No files have been shared yet.</p>}{files.some((file) => file.category === "approval" && file.pending_approval) && <textarea className="approval-note" value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Optional approval or change note" />}</section>
