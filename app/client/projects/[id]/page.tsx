...
export default function ProjectDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PortalShell><section className="portal-content">
    ...
    {!project ? <p className="portal-loading">Loading project…</p> : <>
      ...
      <div className="portal-grid"><section className="portal-section"><h2>Files</h2>{files.length ? files.map((file) => <div className="file-row" key={file.id}><span><strong>{file.filename}</strong><small>{file.category.replaceAll("_", " ")}</small></span><div><button onClick={() => void openProtectedFile(file.download_url)}>Download</button>{file.preview_url && <button onClick={() => { if (file.preview_url) void openProtectedFile(file.preview_url); }}>Preview</button>}{file.category === "approval" && file.pending_approval && <><button onClick={() => void respondToApproval(file.id, "approved")}>Approve</button><button onClick={() => void respondToApproval(file.id, "changes_requested")}>Request changes</button></>}</div></div>) : <p className="portal-muted">No files have been shared yet.</p>}{files.some((file) => file.category === "approval" && file.pending_approval) && <textarea className="approval-note" value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Optional approval or change note" />}</section>
